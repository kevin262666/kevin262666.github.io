...
<template>
  ...
  <section class="block">
    <h2 class="tit-type1" title="設定移動出場與下單條件" :class="{'buyType': radio1=='買進', 'sellType': radio1=='賣出'}">
      設定<b>移動出場</b>與下單條件
    </h2>
          ...
          <el-alert type="info" :closable="false" class="resetAlert">
            <template #title>
              ...
              <div class="resetAlert-content">
                啟動說明：當價格<b class="text-red">≥---</b>時，啟動此移動出場功能。
              </div>
            </template>
          </el-alert>
        </el-col>
        ...
        <el-col :md="12" :span="24">
          <div class="setConditions-item alignTop">
            ...
            <div class="setConditions-item-input">
              <el-row :gutter="store.state.setConditionsItemInputGutter">
                <el-col :span="10">
                  <el-select v-model="value0" placeholder="請選擇">
                    <el-option v-for="item in options0" :key="item.value" :label="item.label" :value="item.value" />
                  </el-select>
                </el-col>
                <el-col :span="14">
                  <el-input-number v-model="num0" :min="1" :step="1" placeholder="TICK" />
                </el-col>
              </el-row>
            </div>
          </div>
  ...
  <section class="block" v-if="value4 != '2'">
    <h2 class="tit-type1 sellType" title="設定停損觸發與下單條件">
      設定<b>停損</b>觸發與下單條件
    </h2>
    <div class="setConditions sellType">
      <el-row :gutter="store.state.setConditionsGutter">
        <el-col :md="12" :span="24">
      ...
      <ul class="confirmDialog-infoList">
        ...
        <li>
          <div class="text-center text-orange">
            移動出場、停損任一條件符合即委託，<br v-if="store.state.windowWidth > 991"/>並停止監控另一個條件。
          </div>
        </li>
      </ul>
      <el-alert type="info" :closable="false" class="resetAlert">
        <template #title>
          ...
          <div class="resetAlert-content">
            在<b class="text-black">2022/04/11</b>前，當<b class="text-black">台積電2330</b>現價≥<b class="text-black">650元後</b>，啟動監控區間高點回檔超過<b class="text-black">5Tick</b>以<b class="text-black">市價</b>委託賣出<b class="text-black">5張</b>；或當<b class="text-black">成交價≤ 515元</b>，即以<b class="text-black">市價</b>停損委託賣出<b class="text-black">5張</b>。
          </div>
        </template>
      </el-alert>
