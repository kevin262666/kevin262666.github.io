	...
	template: ` <nav class="navBar" :class="{show: navbarShow}">
					<ul>
						<li :class="{active : (activeNumber == 0)}">
							<a href="index.html#questionnaire" title="國民理財大調查" class="focusLink">
								國民理財大調查
							</a>
						</li>
						<li :class="{active : (activeNumber == 1)}">
							<a href="index.html#newThought" title="國民理財新思維">
								國民理財新思維
							</a>
						</li>
...
var content = new Vue({
	...
	methods: {
		...
		scrollMagic() {
			...
			bannerTimeLine.add(
				TweenMax.from('.banner-mascot-1', 0.5, {
					y: 1000,
					scale: 70,
					opacity: 0,
				}),
			).add(
				TweenMax.from('.banner-mascot-2', 0.2, {
					x: 100,
					y: 100,
					opacity: 0,
				}),
			).add(
				TweenMax.from('.banner-mascot-3', 0.2, {
					x: -100,
					y: 100,
					opacity: 0,
				}),
			).add(
				TweenMax.from('.banner-bg', 0.3, {
					rotation: 90,
					scale: 0,
					opacity: 0,
				}), 0.6
			).add(
				TweenMax.from('.banner-textTattoo', 0.2, {
					scale: 20,
					opacity: 0,
				}),
			).add(
				TweenMax.from('.banner-tit-1', 0.3, {
					x: -400,
					y: 50,
					opacity: 0,
				}), 0.6
			).add(
				TweenMax.from('.banner-tit-2', 0.3, {
					x: -400,
					opacity: 0,
				}), 0.7
			).add(
				TweenMax.from('.banner-bottom h2', 0.3, {
					y: 400,
					opacity: 0,
				}),
			).add(
				TweenMax.from('.banner-bottom .btnArea', 0.3, {
					y: 400,
					opacity: 0,
				}), 1.2
			);
