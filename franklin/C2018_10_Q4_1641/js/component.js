// ========= 魔法陣 ==============================================================================================================================
Vue.component('magic-svg',{
    template:  `<svg class="magicSvg" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    ...
                    <rect x="73.5" y="74.5" transform="matrix(0.7071 0.7071 -0.7071 0.7071 245.7071 -101.1894)" class="st0" width="343" height="343"/>
...
// ========= 區塊標題 ==============================================================================================================================
Vue.component('section-tit', {
    props: ['section_tit'],
    template:  `<h2 class="tit-1" :class="{type2 : section_tit.type }" :title="section_tit.seotitle">
                    <div class="tit-1-stit" v-html="section_tit.stit"></div>
                    <div class="tit-1-tit" v-html="section_tit.tit"></div>
                </h2>`
});

// ========= 圖表 ==============================================================================================================================
Vue.component('img-chart', {
    props: ['img_chart_list'],
    template:  `<div>
                    <div class="infoItem-text" v-html="img_chart_list.text"></div>
                    <div class="chart-tit" v-html="img_chart_list.chartTit" v-if="img_chart_list.chartTit.length > 0"></div>
                    <img :src="img_chart_list.img" alt="" class="chart-img">
                    <div class="chart-remark" v-html="img_chart_list.remark"></div>
                </div>`
});
...
var content = new Vue({
    ...
    data: {
        ...
        windowWidth: 0,
        windowHeight: 0,
        accessDatabase: null, // 取google表單資料
        userAgent: null, // 取得浏览器的userAgent字符串
        sectionTit: [{
                type: false,
                seotitle: '不只住宅要防震，投資也要會避震！',
                stit: '不只住宅要防震',
                tit: '<strong>投資也要會避震！</strong>',
            },
            {
                type: true,
                seotitle: '為何市場風起雲湧之際，正是另類資產大顯身手之時？',
                stit: '為何市場風起雲湧之際',
                tit: '<strong>正是另類資產大顯身手之時？</strong>',
            },
            {
                type: false,
                seotitle: '不懂另類資產？搞定四大策略就行',
                stit: '不懂另類資產？',
                tit: '<strong>搞定四大策略就行</strong>',
            },
            {
                type: true,
                seotitle: '擁有對沖策略 對你的好處是？',
                stit: '擁有對沖策略',
                tit: '<strong>對你的好處是？</strong>',
            },
            {
                type: false,
                seotitle: '想找另類資產？ 優質選擇就在這裡！',
                stit: '想找另類資產？',
                tit: '<strong>優質選擇就在這裡！</strong>',
            },
            {
                type: true,
                seotitle: '投資密碼',
                stit: '',
                tit: '<strong>投資密碼</strong>',
            },
            {
                type: false,
                seotitle: '申購優惠',
                stit: '',
                tit: '<strong>申購優惠</strong>',
            },
            {
                type: true,
                seotitle: '精選基金',
                stit: '',
                tit: '<strong>精選基金</strong>',
            },
        ],
        imgChartList: [
            {
                text: '隨著貿易戰風波未平，市場的波動恐隨著消息紛擾而逐漸擴大，佈局另類資產，有助降低投資波動。',
                chartTit:  `<strong>
                                對沖策略並非外界認知過於積極和冒險，反而更專注於低波動下的資產增值，其波動度僅有世界指數的50%
                            </strong>`,
                img: 'images/chart1.svg',
                remark: '資料來源: 理柏資訊，統計過去10年期間，截至2018/7/31，波動風險為過去三年月報酬率的標準差年化值。',
            },
            {
                text: '全球央行開始偏向緊縮，市場利率走高，債市投資首當其衝。然而，就過去經驗，具有對沖策略的另類資產卻能在利率上揚的環境中，平衡股債同步走跌的風險，爭取超額報酬的機會。',
                chartTit: `<strong>
                                過往統計:對沖策略的超額報酬受益於利率的走升
                            </strong>`,
                img: 'images/chart2.svg',
                remark: '資料來源:來自富蘭克林坦伯頓基金集團，彭博資訊，統計1991年1月至2018年5月數值。取HFRI對沖基金綜合指數(HFRIFWI)為例，超額報酬以對應美國史坦普500指數為統計。',
            },
            {
                text: '全球央行開始偏向緊縮，市場利率走高，債市投資首當其衝。然而，就過去經驗，具有對沖策略的另類資產卻能在利率上揚的環境中，平衡股債同步走跌的風險，爭取超額報酬的機會。',
                chartTit: ``,
                img: 'images/chart3.svg',
                remark: '資料來源: 富蘭克林坦伯頓基金集團、彭博資訊，2017/10/1至2018/8/28，以2017/10/1的指數價位為基礎點(100)。上述不代表任一個股之推薦。',
            },
            {
                text: '2018年6月基金經理人買進時代華納的股票，並同步做空AT&T電信公司，當6月12日美國法院批准AT&T收購時代華納的交易，隔日時代華納上漲1.8%，AT&T下跌6.2%，顯示策略奏效。',
                chartTit: ``,
                img: 'images/chart4.svg',
                remark: '資料來源: 彭博資訊，富蘭克林坦伯頓基金集團。上述不代表任一個股之推薦。',
            },
            {
                text: '一般來說，高收益債和投資等級債的利差水準會維持在相對應的均衡區間。但今年美國經濟熱絡表現，使美國高收益債在過去六個月收斂10個基本點bps，相較之下，投資級公司受貿易糾紛影響大，利差反而上升10個基本點bps(截至8/28)，因此，在債券間表現差異加大時，就提供基金經理人相對價值的套利機會。',
                chartTit: ``,
                img: 'images/chart5.svg',
                remark: '資料來源: 彭博資訊，皆使用彭博巴克萊債券指數做計算。計算期間：2018/3/1-2018/8/28',
            },
            {
                text: '今年以來，部份新興國家因受地緣政治、中美貿易戰和經濟數據表現不佳，提供基金經理人放空部份體質較弱的新興國家貨幣，爭取匯兌收益機會。',
                chartTit: ``,
                img: 'images/chart6.svg',
                remark: '資料來源: 彭博資訊，富蘭克林坦伯頓基金集團。標準化於2018/1/1。',
            },
            {
                text: '在股市或債市下跌年度，對沖策略展現抗跌及逆勢上漲的表現。',
                chartTit: ``,
                img: 'images/chart7.svg',
                remark: `資料來源：彭博資訊、HFR、巴克萊。 取摩根士丹利全球股市指數、彭博巴克萊美國綜合債券指數、HFRI對沖基金綜合指數為例。
                            <br/>註： HFRIFWI的成份為全球性、 相同權重、 超過2000檔有匯報資料給HFR公司資料庫的基金進行統計。 有別於多數資產類別， HFR指數有反應費用及相關支出。`,
            },
            {
                text: '相較於傳統股債配置的投資組合，若在組合納入另類資產，能以較低的波動風險，提供較高的報酬機會。',
                chartTit: ``,
                img: 'images/chart8.svg',
                remark: `資料來源：FactSet，計算期間過去20年截至2018年6月30日之年化報酬率與年化波動風險，美元計價，取HFRI對沖基金綜合指數、摩根士丹利全球股市指數、彭博巴克萊全球綜合債券指數所試算。`,
            },
        ],
        menuBtnActive: false,
        navbarShow: false,
    ...
    mounted() {
        // 取google表單資料
        axios
            .get('https://spreadsheets.google.com/feeds/cells/1kKGcZV9W2L36WtogO2DG4GxBs1TjqeZDo14PUyd8nKM/8/public/values?alt=json')
            .then(response => (
                this.accessDatabase = response.data.feed,

                // sectionTit
                this.sectionTit[1].seotitle = response.data.feed.entry[7].content.$t + '，' + response.data.feed.entry[9].content.$t,
                this.sectionTit[1].stit = response.data.feed.entry[7].content.$t,
                this.sectionTit[1].tit = '<strong>' + response.data.feed.entry[9].content.$t + '</strong>',

                this.sectionTit[2].seotitle = response.data.feed.entry[32].content.$t + '，' + response.data.feed.entry[34].content.$t,
                this.sectionTit[2].stit = response.data.feed.entry[32].content.$t,
                this.sectionTit[2].tit = '<strong>' + response.data.feed.entry[34].content.$t + '</strong>',

                this.sectionTit[3].seotitle = response.data.feed.entry[67].content.$t + '，' + response.data.feed.entry[69].content.$t,
                this.sectionTit[3].stit = response.data.feed.entry[67].content.$t,
                this.sectionTit[3].tit = '<strong>' + response.data.feed.entry[69].content.$t + '</strong>',

                this.sectionTit[4].seotitle = response.data.feed.entry[88].content.$t + '，' + response.data.feed.entry[90].content.$t,
                this.sectionTit[4].stit = response.data.feed.entry[88].content.$t,
                this.sectionTit[4].tit = '<strong>' + response.data.feed.entry[90].content.$t + '</strong>',

                // imgChartList
                this.imgChartList[0].text = response.data.feed.entry[16].content.$t,
                this.imgChartList[0].chartTit = '<strong>' + response.data.feed.entry[18].content.$t + '</strong>',
                this.imgChartList[0].remark = response.data.feed.entry[20].content.$t,

                this.imgChartList[1].text = response.data.feed.entry[25].content.$t,
                this.imgChartList[1].chartTit = '<strong>' + response.data.feed.entry[27].content.$t + '</strong>',
                this.imgChartList[1].remark = response.data.feed.entry[29].content.$t,

                this.imgChartList[2].text = response.data.feed.entry[41].content.$t,
                this.imgChartList[2].remark = response.data.feed.entry[43].content.$t,

                this.imgChartList[3].text = response.data.feed.entry[48].content.$t,
                this.imgChartList[3].remark = response.data.feed.entry[50].content.$t,

                this.imgChartList[4].text = response.data.feed.entry[55].content.$t,
                this.imgChartList[4].remark = response.data.feed.entry[57].content.$t,
                
                this.imgChartList[5].text = response.data.feed.entry[62].content.$t,
                this.imgChartList[5].remark = response.data.feed.entry[64].content.$t,

                this.imgChartList[6].text = response.data.feed.entry[76].content.$t,
                this.imgChartList[6].remark = response.data.feed.entry[78].content.$t,

                this.imgChartList[7].text = response.data.feed.entry[83].content.$t,
                this.imgChartList[7].remark = response.data.feed.entry[85].content.$t
            ));
        
        this.$nextTick(() => {
            //-------------------------------------signature
            console.log('%cMade by Captain%c2018/10%chttps://www.behance.net/kevin2626634eb',
